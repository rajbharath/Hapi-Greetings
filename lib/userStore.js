...
UserStore.initialize = function() {
  UserStore.createUser('Bharath', 'abcd@gmail.com', 'password');
};
  ...
  bcrypt.genSalt(10, function(err, salt) {
    bcrypt.hash(password, salt, function(err, hash){
      ...
      if(UserStore.users[email]) {
        callback(Boom.conflict('Email already present. Please login'));
      }
      else {
...
UserStore.validateUser = function(email, password, callback) {
  ...
  if (!user) {
    callback(Boom.notFount('User does not exist'));
    return;
  }
